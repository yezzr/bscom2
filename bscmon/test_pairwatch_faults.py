...
def rpc_logfail(urls, method, params, tries=2):
    if method == "eth_getLogs":
        _n["i"] += 1
        return [] if _n["i"] == 1 else None            # chunk1 ok(empty), chunk2 FAILS
    return None
pw._rpc = rpc_logfail
out, scanned = pw.get_paircreated(0, 99)               # CHUNK=50 -> chunk1=0-49, chunk2=50-99(fail)
check("stops before failed chunk (scanned==49, not 99)", scanned == 49)
...
pw.get_paircreated = lambda frm, to: ([(BASE, T_BURN, P_BURN)], to)
...
pw.get_paircreated = lambda frm, to: ([], to)
_code[T_BURN] = BURN
pw.main()
st = load_state()
...
HEAD[0] = 2000
pw.head_block = lambda: HEAD[0]
pw.get_paircreated = lambda frm, to: ([(BASE, T2, P2)], to)
